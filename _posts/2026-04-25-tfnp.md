...
- Small note on CLS: Given a function that's continuous and differnetiable ($\mathcal{C}^1$), the CLS-complete problem shown by [5] is to find the points where Gradient Descent on this function terminates. The nuance is performing gradient descent on a $\mathcal{C}^1$ function is an algorithm; finding the points where this algorithm terminates is a search problem.
