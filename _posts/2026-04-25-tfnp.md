...
Continuing on the [PPAD]({% post_url 2026-01-27-ppad %}) train of thought, I want to examine other problems in the TFNP zoo. As I'm personally trying to make sense of this whole thing, we're first going to define TFNP formally.
...
- Small note on CLS: Given a function that's continuous and differnetiable on a bounded domain ($\mathcal{C}^1$), the CLS-complete problem shown by [5] is to find the points where Gradient Descent on this function terminates. The nuance is performing gradient descent on this function is an algorithm; finding the points where this algorithm terminates for this function is a search problem.
